...
### A12. `share_basis` admits `cross_reference` — a share fixed by renvoi is not a deferred share (GHA, batch 8, 2026-08-20)

- **Choice.** Dictionary field 20a `share_basis` is extended additively (CORE C10) to
  `{stated, whole_proceeds_implied, deferred_arrete, cross_reference, statutory_maximum}`.
  `cross_reference` records a share fixed by statutory renvoi to another instrument's *existing*
  text — the value lives in a cited instrument, neither stated in-clause nor deferred to a future
  subsidiary one. `share_value` stays null; no number is ever imported from the cited instrument.
  This restores the symmetry the dictionary already claims: v0.4 added `cross_reference` to
  `rate_basis` (16a) for exactly this reason and 16a's own entry calls itself "symmetric to
  `share_basis`", but the matching extension to 20a was never made.
- **Forced by.** GHA `EST_NATIONALHEALTHINSURANCE_2012` s.41(1)(h) and s.41(4) — "moneys accruing
  under section 198 of the Insurance Act, 2006 (Act 724)". Previously surfaced independently on
  three batch-7 documents (`EST_MINERALSDEVELOPMENTFUND_2016` s.18(a),
  `EST_MINERALSINCOMEINVESTMENTFUND_2018` s.31(1), `EST_GHANAEDUCATIONTRUSTFUND_2018` s.4(2)) and
  resolved three different ways, which is what forced a central ruling.
- **Load-bearing because.** `deferred_arrete` was the available near-miss and is definitively wrong:
  its definition is a share deferred to a *future subsidiary* instrument, and a renvoi to an existing
  primary Act is neither future nor subsidiary. Recording it there would reproduce the exact error
  v0.4 corrected on the rate side. The distinction is not cosmetic for identification: a
  `deferred_arrete` share is a share whose *value is set by the executive*, and a reallocation set by
  subsidiary instrument for reasons unrelated to a sector's outcomes is precisely the cleaner
  exogeneity claim recorded at A2. Pooling renvoi shares into that bucket would contaminate the
  treatment variable with shares that no executive ever set.
- **Constrains.** A `cross_reference` share is UNVALUED until reconciliation resolves the citation
  against the cited instrument. Any share-level statistic must state whether `cross_reference` rows
  are excluded or imputed; they must never be silently read as zero or as whole proceeds. The
  resolution itself is a separate logged pass — extraction is forbidden from opening the cited Act.
- **Boundary.** A **broken** cross-reference (the cited text does not contain the share) is NOT
  `cross_reference`: record `share_basis: null`, `ai_confidence: medium`, and note the stale pointer
  (CORE C8, no-invention). Attested on `EST_GHANAEDUCATIONTRUSTFUND_2018` s.4(2).
- **Housekeeping.** The pre-ruling `EST_MINERALSDEVELOPMENTFUND_2016` EV_0009 row already carries
  `share_basis: "cross_reference"` as an unratified C10 extension; this entry retrospectively
  ratifies it. That file was not reopened in batch 8.

### A13. `rate_basis` and `share_basis` admit `statutory_maximum` — a bound is not a charge (GHA, batch 8, 2026-08-20)

- **Choice.** Both bases are extended additively (CORE C10) with `statutory_maximum`, for enacting
  text that prints a numeral as a **bound** rather than as the charge imposed ("not exceeding",
  "not more than", "up to", and the floor-and-ceiling form "not less than X and not more than Y").
  Encoding is identical on both sides: scalar **null**; basis `statutory_maximum`; `*_is_schedule = 1`;
  the bound **verbatim** in `*_schedule_detail`, including both limbs of a range; `*_type` as written.
- **Forced by.** GHA `EST_STUDENTSLOANTRUSTFUND_2011` s.3(a) — "up to ten per cent of the inflows
  into the Ghana Education Trust Fund", the corpus's **first share-side** occurrence — and
  `EST_NATIONALHEALTHINSURANCE_2012` ss.27(13), 63(2), 65(2), 107(1)–(4), 108(2), four of which are
  floor-and-ceiling ranges. First raised on `CTA_AIRTAX_1963` ss.3(2), 6 in batch 7 and handled there
  by an interim that this entry supersedes.
- **Load-bearing because.** Each available alternative misstates the mechanism or loses the number.
  `stated` + the numeral asserts a fixed charge the statute does not impose — prohibited by decision
  rules §6, which forbids coercing a value into a column that would fabricate information. `banded`
  means a schedule keyed on an attribute (price band, substance, population) and a bare bound is
  keyed on nothing. `not_applicable` — the batch-7 interim — is the costly one: it renders every
  ceiling invisible to quantitative use, and statutory maxima are ubiquitous in Anglophone drafting
  while this corpus carries a literal penalty grain, so the null would have propagated across most
  penalty rows in the corpus.
- **Constrains.** A `statutory_maximum` row records a **bound, not a realised charge**, so it must
  never enter a rate or share panel as a level. The bound is machine-findable in `*_schedule_detail`
  but is not parsed at extraction; parsing it into a child table (bound, floor, unit) is deferred to
  R along with the other `*_schedule_detail` fields (dictionary §6). Until that parse exists, any
  aggregate over penalty rows must state that maxima are excluded.
- **Boundary.** A pure floor with no ceiling ("at least forty percent", "not less than five per
  cent") is NOT `statutory_maximum`. The batch-7 precedent governs it unchanged: a floor is not a
  share and its complement is not a number — `share_value: null`, quantum verbatim in
  `share_schedule_detail`, and the residual recorded as no number anywhere, never back-computed.
- **Open, not closed by this entry.** The dictionary still has **no column for a cap as distinct from
  a rate**. `statutory_maximum` makes the bound findable and correctly typed; it does not make it a
  first-class quantity. If penalty severity is ever an outcome or a covariate, a dedicated cap column
  is the next additive step.
